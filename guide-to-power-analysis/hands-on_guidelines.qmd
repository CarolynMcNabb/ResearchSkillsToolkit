...
library(lme4)                    # perform linear mixed model analysis
```
...
-   Studies designed with high statistical power yield narrower 95% confidence intervals (CI). This improves precision and reduces the uncertainty around effect sizes estimates. @fig-figure3 depicts the confidence intervals from five replicated studies conducted with two different sample sizes (N of 40 and N of 200), to detect a true effect size of 0.3.
...
```{r tab-table1, tab.cap = "Achieved power for an unpaired t-test assuming different correlations."}
# Set parameters
nsims <- 1000            # number of simulations
N <- 50                  # sample size
...
This study design would require a sample size of `r ceiling(adj$n)` to achieve a power of `r adj$power` with an alpha of **`r adj$sig.level`**.
...
With `r ceiling(no_adj$n)` participants per group, *d* = `r no_adj$d`, and alpha = `r no_adj$sig.level`, this design achieves `r no_adj$power*100`% power without correction.
...
This shows that the study design would be underpowered once multiple-comparison correction is applied post hoc.

# Effect size justification {#sec-es-justification}
...
Another method to account for uncertainty and potential inflation of effect size estimates reported in the literature is to adjust the effect size using the package *`BUCSS`* [@bucss_software]. This approach allows researchers to correct for uncertainty and bias when planning their sample size. For a detailed discussion on how to adjust inflated effect size estimates using the package `BUCSS` , readers are referred to @anderson_sample_planning and the [package documentation](https://cran.r-project.org/web/packages/BUCSS/BUCSS.pdf). Below we demonstrate how to use the BUCSS package to account for publication bias and uncertainty in the context of an unpaired *t*-tests and a two-way mixed ANOVA with one between-subject and one within-subject factor. For additional supported tests, consult [package documentation](https://cran.r-project.org/web/packages/BUCSS/BUCSS.pdf).
...
# Conducting an a priori power analysis {#sec-power-analysis}
...
To estimate the power for factorial designs, researchers can use the package `Superpower` to set up the factorial design using `ANOVA_design()` and perform the simulations using `ANOVA_power()` analyses for main and interaction effects as well as planned pairwise comparisons. A comprehensive tutorial on using `Superpower` package can be found in [Power Analysis with Superpower](https://aaroncaldwell.us/SuperpowerBook/).

A more flexible simulation-based approach involves using `faux::sim_design()` [@debruine_faux] to simulate data and applying the relevant test to estimate power. This approach is particularly flexible, as it enables researchers to simulate data not only for basic (i.e., paired- and unpaired-sample designs) and factorial designs, but also for more complex designs such as multilevel models. For a comprehensive tutorial on using `faux` package, readers are referred to <https://debruine.github.io/faux/index.html>. For a gentle introduction to conducting a power analysis using simulations, readers are also referred to [Nick's post](https://nickch-k.github.io/EconometricsSlides/Week_08/Power_Simulations.html) and [LMU Open Science Center](https://lmu-osc.github.io/Simulations-for-Advanced-Power-Analyses/LM1.html).
...
Suppose a researcher hypothesizes that the difference in reaction time between morning and evening will correspond to a Cohen’s *d~rm~* of 0.2. The researcher plans to collect data from the same participants at both time points.
...
For this study design, we would need a sample size of `r ceiling(pwr_paired$n)` to achieve `r pwr_paired$power`.
...
For this study design, we would need a sample size of `r ceiling(pwr_unpaired$n)` to achieve `r pwr_unpaired$power`.
...
Let's perform the simulations using `t.test()`:
...
## Designs with more than two levels
...
```{r chunk_36}
nsims <- 1000                         # number of simulations
sample_size <- c(25, 50, 75, 100)     # total sample size
mu <- c(60, 62, 65)                   # expected group means per for A, B, C
sd <- 10                              # common SD
rho <- 0.6                            # correlation between measurements
```

### Simulation using `aov()`

Before running the simulation, always make sure that the pattern of means matches the expected pattern of means:

```{r chunk_37, results='hide'}
sim_design(
...
simulation_result <- ANOVA_power(
  ...
  nsims = nsims, 
  verbose = FALSE,
  seed = 050990) # for reproducibility
```
...
Although the *F*-test has adequate power to reject the null hypothesis of no difference across conditions (i.e., power = `r simulation_result$power`), only the pairwise comparison between interventions A and C has adequate power (i.e., power = `r simulation_result$pc_results[2,1]`. Because the difference in heart rate between A and C is larger than that between A and B, the pairwise comparison test for A vs C achieves higher power for any given sample size. Therefore, a larger sample size would be needed to attain comparable power for the A vs B comparison.
...
```{r chunk_43, results='hide'}
# Set parameters 
nsims <- 1000                    # number of simulations
sample_size <- c(50, 100, 150)   # sample size per group
mu <- c(60, 62, 64)              # expected group means 
sd <- 10                         # common SD
```
...
power_results <- map_dfr(sample_size, simulate_power)
```
...
power_results %>%
  ...
  select(N, power) %>%
  kable(digits = 2, caption = "Power for high dose vs. placebo") %>%
  kable_styling(full_width = FALSE)
```
...
power_results %>%
  ...
  select(N, power) %>%
  kable(digits = 2, caption = "Power for medium dose vs. placebo") %>%
  kable_styling(full_width = FALSE)
```
...
The study design would achieve a power of `r pwc[1, 2]` to test the comparison of intervention A between morning and evening, a power of `r pwc[2, 2]` to test for the comparison between interventions A and B in the morning and a power of `r pwc[3, 2]` to test for the comparison between interventions A and B in the evening. To ensure adequate power (80%) for the second pairwise comparison, the researcher should consider increasing the sample size.
...
mu <- c(64, 60, 64, 64)                  # means for A_pre, A_post, B_pre, B_post
sd <- 10                                 # common SD
rho <- 0.7                               # correlation between measurements
alpha_level <- 0.05                      # alpha level
```

Let's visualize the pattern of means:

```{r chunk_60,  results='hide'}
sim_design(
      within = list(time = c("pre", "post")),
...
### Simulation using `lm()` including time as a covariate
...
## One-factor between-subject design with a continuous predictor
...
Suppose a researcher hypothesizes that the difference in heart rate between intervention A and B will not exceed 5 bpm, corresponding to a Cohen's *d~s~* of 0.2 (5/25 = 0.2). In this line of research, the SESOI has been set to ± 10, corresponding to a Cohen's *d~s~* of 0.2 (10/25 = 0.4). This means that any observed difference between the two interventions smaller than 0.3 (in either direction) is considered practically equivalent—that is, not large enough to be meaningful in practice. As illustrated below, if the 90% CI entirely within the lower and upper bounds of the SESOI.

```{r, echo=FALSE, fig-figure9, fig.cap = "The grey vertical lines represent the lower and upper bound of the SESOI corresponding to the region of equivalence. The horizontal black line represents the 90% CI of the observed effect."}
...
```{r chunk_70, include=FALSE}
power_t_TOST(alpha = 0.05,
             delta = 2,           # true difference in means
             sd = 10,             # standard deviation of the difference
             n = 50,              #
             eqb = 5,             # set upper and lower bounds
             type = "two.sample")
```
...
```{r fig-figure11, fig.cap = "A priori power analysis to estimate sample size for L2 based on on the summary-statistic approach"}
...
@westfall_2014 created an app to run power analyses for simple designs with one fixed effect and two random factors. The app is available at [https://jakewestfall.shinyapps.io/crossedpower](#0).

### Simulation-based power analysis (work in progress)

An alternative is to use simulation-based power analysis. For a comprehensive tutorial on this approach, readers are referred to @debruine_mixed_effects.
...
```{r fig-figure12, fig.cap = "Screenshot of G*Power interface with all required information to reproduce the power analysis for a one-way within-subject design with three 3 dependent groups.", out.width = "100%"}
